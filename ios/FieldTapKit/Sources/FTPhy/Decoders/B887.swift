...
//
// The field *widths* come from the second capture (2026-09-22), which has 828 new transmissions on a wider NR
// carrier and in the field. It uses more than the first one ever did: up to 106 resource blocks (7 bits was
// one short), 4 layers, and slot numbers above 15. Each width was widened until the values stopped wrapping
// and the TBS identity held again; TBS is the one that narrows, because its top bits belonged to the next
// field all along and the first capture's blocks were too small to ever set them.
...
enum B887 {
    ...
    /// 5-14, slot bits 15-19; u16 @12 PCI 10b; u32 @16 TBS bytes bits 5-22, MCS bits 26-30; u32 @20 nRB bits 0-7,
    /// HARQ bits 11-14, layers-1 bits 29-30; byte @24 bit 0 CRC pass.
    static func decode(_ b: [UInt8]) -> Decoded<[Slot]> {
        ...
        for k in 0..<b.u8(7) {
            ...
            out.append(Slot(frame: w2.bits(5, 10), slot: w2.bits(15, 5), pci: b.u16(r + 12) & 0x3FF,
                            tbsBytes: w4.bits(5, 18), mcs: w4.bits(26, 5), nRb: w5.bits(0, 8), harq: w5.bits(11, 4),
                            layers: w5.bits(29, 2) + 1, crcOk: b[r + 24] & 1 == 1))
        }
        return .value(out)
    }
}
