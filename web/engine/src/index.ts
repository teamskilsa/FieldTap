// OWNER: integrator (seeded by the engine foundation). The engine's only entry point for the UI: the drop-in
// replacement for the Lovable project's src/engine/index.ts. analyzeFile runs the analysis in a Web Worker and
// resolves with a CaptureAnalysis (whose problems explain an unusable file); it rejects with an AbortError when
// `signal` aborts.
...
/** The worker to run in; bundlers (Vite) resolve this URL form into a separate worker chunk. */
...
export function analyzeFile(
  ...
  return new Promise((resolve, reject) => {
    ...
    const onAbort = () => {
      ...
      reject(new DOMException('The analysis was cancelled', 'AbortError'));
    };
    ...
    worker.onmessage = (event: MessageEvent<WorkerReply>) => {
      ...
      if (m.type === 'progress') onProgress(m.progress);
        ...
        reject(m.name === 'AbortError' ? new DOMException(m.message, 'AbortError') : new Error(m.message));
      }
    };
