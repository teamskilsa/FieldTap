...
function syntheticSysdiagnose(extra: TarSpec[] = []): TarSpec[] {
  return [
    ...
    ...Array.from({ length: 30 }, (_, i) => ({ path: `${ROOT}/logs/Other/f${i}.log`, data: new Uint8Array(20_000).fill(i) })),
...
function assertPlainData(v: unknown, path = 'analysis'): void {
  ...
  assert(typeof v === 'object' && Object.getPrototypeOf(v) === Object.prototype, `${path} is not a plain object (${typeof v})`);
...
const REQUIRED: (keyof CaptureAnalysis)[] = [
  'contract', 'fileName', 'traceWindow', 'profile', 'guide', 'problems', 'records', 'codes', 'encryptedRecords', 'encrypted',
  'crcErrors', 'durationMs', 'events', 'procedures', 'steps', 'connections', 'cellDetails', 'ladder', 'journey', 'phy',
  'phySummary', 'phyChecks', 'versionMisses', 'availability', 'timings',
];
...
async function run(bytes: Uint8Array, fileName: string, sizes = [8192], earlyStop = true) {
  ...
  const a = await analyzeArchive(streamOf(bytes, sizes), (p) => progress.push(p), undefined, { fileName, totalBytes: bytes.length, nowMs: PRESS, earlyStop });
...
Deno.test('analyzeArchive: a synthetic sysdiagnose gives the archive facts, a complete shape, and staged progress', async () => {
  ...
  assertEquals(a.traceWindow, {
    startUtc: '2026-09-21T19:42:06.000Z', endUtc: '2026-09-21T19:42:33.844Z', afterPressStartS: 19, afterPressEndS: 46.844,
    filesKept: 3, filesOnPhone: 3, filesOverwritten: 0, filesMissing: 0,
  });
  assertEquals([a.profile.status, a.profile.removalDate, a.guide.status], ['active', '2026-09-28T19:40:02.000Z', 'active']);
  ...
  for (const s of ['reading', 'extracting', 'deframing', 'decoding', 'radio'] as const) assert(typeof a.timings[s] === 'number', `timing ${s}`);
});
...
Deno.test('analyzeArchive: unusable files resolve with a blocking problem, never a rejection', async () => {
  ...
  const off = await gzip(buildTar([{ path: `${ROOT}/logs/Baseband/ambtool_output.log`, data: 'Baseband logs are not enabled\n' }]));
  const offRun = await run(off, 'renamed.tar.gz');
  assertEquals(offRun.a.problems.map((p) => p.kind), ['loggingNotEnabled', 'noBasebandTrace']);
  assertEquals([offRun.a.guide.status, offRun.a.triggerTime], ['off', '2026-09-21T19:41:47.000Z'], 'press from the root directory');
});
...
Deno.test('analyzeArchive: cancelling rejects with an AbortError', async () => {
  ...
  await assertRejects(
    () => analyzeArchive(streamOf(gz, [256]), () => controller.abort(), controller.signal, { fileName: 'x.tar.gz', totalBytes: gz.length }),
