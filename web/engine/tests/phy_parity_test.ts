...
import { CENSUS, endsDiff, type Golden, loadCapture, PHY_GOLDEN, PHY_SUMMARY, QMDL, statsDiff, TBS_REFERENCE } from './phy_support.ts';
...
Deno.test({
  ...
  fn: async () => {
    ...
    assertEquals([...Object.keys(kpis)].sort(), [...PHY_METRIC_ORDER].sort(), 'the golden names every PhyMetric');
    assertEquals(run.capture.series.map((s) => s.metric), PHY_METRIC_ORDER, 'one series per metric, in PhyMetric order');
    const diffs: string[] = [];
    for (const s of run.capture.series) {
      ...
      if (g.perIndex) {
        g.perIndex.forEach((want: Golden, i: number) => {
          const v = s.samples.flatMap((x) => (x.perIndex && x.perIndex[i] !== null && x.perIndex[i] !== undefined ? [x.perIndex[i]!] : []));
      ...
      if (s.metric === 'lte_cqi_wideband_cw0' || s.metric === 'lte_ri' || s.metric === 'lte_pmi_wideband') {
        ordered = [...s.samples.filter((x) => x.tag === TAG.puschCsf), ...s.samples.filter((x) => x.tag === TAG.pucchCsf)];
      }
    ...
    assertEquals(run.stats.malformed, {});
    assertEquals(run.stats.unstamped, {}, 'no PHY record needs an interpolated time');
  },
});
...
Deno.test({
  ...
  fn: async () => {
    ...
    got.scellActivity.forEach((a, i) => {
      ...
      assertEquals([a.index, a.earfcn, a.pci, a.records, a.source], [b.index, b.earfcn, b.pci, b.records, b.source], `scell ${i}`);
...
Deno.test({
  ...
  fn: async () => {
    ...
    assertEquals(s.nr, { b887Records: 497, b887CrcFail: 27, b887PassBytes: 653_592, deltaDecodes: 497, deltaCrcFail: 27, deltaPassBytes: 653_592 });
    ...
    assertEquals(checks.map((c) => c.id), ['b193RsrqIdentity', 'b173TbsTable', 'b139TbsModulation', 'b887TbsFormula', 'b887VsB888', 'b888CounterIdentity', 'b064HeaderAccounting']);
...
Deno.test({
  ...
  fn: async () => {
    ...
    assertEquals(shipped.checks.map((c) => c.id), ['b193RsrqIdentity', 'b887TbsFormula', 'b887VsB888', 'b888CounterIdentity', 'b064HeaderAccounting']);
    ...
    const counts = Object.fromEntries(shipped.availability.map((a) => [a.id, /This capture: ([\d,]+) records/.exec(a.reason)?.[1] ?? null]));
    assertEquals([counts.nrUlSchedule, counts.nrUlPower, counts.nrDci, counts.nrCsf, counts.b134], ['633', '605', '632', '42', '4,888']);
  },
});
...
Deno.test({
  ...
  fn: async () => {
    ...
    assertEquals([by.nr_dl_mcs.section, by.nr_dl_mcs.version, by.nr_dl_mcs.unit], ['nr', '3.13', 'index (qam256 table)']);
    ...
    for (const m of ['lte_dl_mcs', 'lte_ul_prb', 'lte_cqi_wideband_cw0', 'lte_mac_ul_grant', 'lte_dl_bler', 'nr_dl_mcs'] as const) {
