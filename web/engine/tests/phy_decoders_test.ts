...
import { decodeB126 } from '../src/phy/decoders/b126.ts';
import { decodeB12A } from '../src/phy/decoders/b12a.ts';
import { decodeB16C } from '../src/phy/decoders/b16c.ts';
import { decodeB179 } from '../src/phy/decoders/b179.ts';
import { bindingLimitDbm, decodeX184C } from '../src/phy/decoders/fedTxAgc.ts';
import { decodeX1D0B } from '../src/phy/decoders/modemClock.ts';
...
function b193(
  cells: { earfcn: number; pci: number; serving: boolean; carrier: number; rxMap: number; rsrp: number }[],
  subVersion = 66,
): Uint8Array {
  ...
  for (const [earfcn, list] of byEarfcn) {
    ...
    for (const c of list) {
      ...
      w[10] = s | (s << 11);
      w[11] = s;
      const cell = new Bytes().u32(c.rxMap).zeros(4).u16(c.pci | (c.carrier << 9) | (c.serving ? 0x8000 : 0)).zeros(2)
        .u16(0).zeros(10);
...
function b173(
  carrier: number,
  blocks: { mcs: number; nrb: number; tbs: number; qm: number; crc: boolean }[],
  layers = 1,
  version = 50,
): Uint8Array {
...
function b139(carrier: number, nRb: number, tbsBytes: number, modulation: number, powerRaw: number): Uint8Array {
  const r = new Bytes().u16(1_234).u16(carrier).u32((5 << 1) | (nRb << 15)).u16(tbsBytes).u16(512).zeros(24).u8(
    modulation << 2,
  ).zeros(9).u8(powerRaw);
...
const b14e = (carrier: number, ri: number, cqi0: number, cqi1: number, pmi: number) =>
  new Bytes().u8(164).u32((carrier << 14) | ((ri - 1) << 28)).u32((cqi0 << 7) | (cqi1 << 11) | (pmi << 24)).u8(4)
    .done();

const b14d = (type: number, word6: number, word10: number) =>
  new Bytes().u8(164).u32(type << 26).u8(0).u16(word6).u16(4).u16(word10).u16(0).done();

function b064(grant: number, header: number[]): Uint8Array {
  const sample = new Bytes().u8(0, 3, 0).u16((10 << 4) | 2).u16(grant).u8(0).u16(0).u8(0, 0, header.length).u8(
    ...header,
  );
...
function b062(ta: number | null, target = -110, ulEarfcn = 19_000, subVersion = 50): Uint8Array {
  const body = new Bytes().u8(6, 0, 1, 0, 1, ta === null ? 1 : 3).u8(12, 0).u16(target & 0xffff).zeros(3).u16(0).u8(0)
    .u16(0).u16(ta ?? 0);
...
function b97f(
  ...
  ccId: number,
  servingPci: number,
  cells: { pci: number; rsrp: number; rsrq: number; beams: number }[],
  minor = 0,
): Uint8Array {
  ...
  for (const c of cells) {
    out.u16(c.pci).u16(0).u8(c.beams).zeros(3).u32(q7raw(c.rsrp)).u32(q7raw(c.rsrq)).zeros(84 * c.beams);
  }
  return out.done();
}

function b887(
  slots: { mcs: number; nrb: number; tbs: number; layers: number; crc: boolean; pci?: number; slot?: number }[],
  minor = 13,
): Uint8Array {
...
Deno.test('phy decoders: LTE MIB and serving cell info read their fields; other versions are misses', () => {
  ...
  assertEquals(value(decodeB0C2(sci)), {
    pci: 7,
    dlEarfcn: 1_000,
    ulEarfcn: 19_000,
    dlBandwidthPrb: 50,
...
Deno.test('phy decoders: 0xB193 per-Rx, combined and filtered measurements, serving flag and SCell index', () => {
  const cells = value(
    decodeB193(
      b193([{ earfcn: 1_000, pci: 7, serving: true, carrier: 2, rxMap: 0b0101, rsrp: -95.5 }, {
        earfcn: 1_000,
        pci: 9,
        serving: false,
        carrier: 0,
  ...
  assertEquals(decodeB193(b193([{ earfcn: 1, pci: 1, serving: true, carrier: 0, rxMap: 1, rsrp: -90 }], 65)), {
    kind: 'versionMiss',
    key: '0xB193 v1/0x19 v65',
    version: 'v1/0x19 v65',
  });
...
Deno.test('phy decoders: 0xB173 transport blocks and 0xB139 PUSCH', () => {
  const [r] = value(
    decodeB173(
      b173(2, [{ mcs: 20, nrb: 25, tbs: 1_000, qm: 6, crc: true }, { mcs: 31, nrb: 25, tbs: 0, qm: 6, crc: false }], 2),
    ),
  );
  assertEquals([r.sfn, r.subframe, r.layers, r.transportBlocks, r.carrier], [100, 3, 2, 2, 2]);
  assertEquals(r.blocks.map((t) => [t.mcs, t.nRb, t.tbsBytes, t.qm, t.crcOk, t.rntiType, t.harq]), [[
    20,
    25,
    1_000,
    6,
  ...
  assertEquals([tx.pci, tx.carrier, tx.nRb, tx.startRb, tx.tbsBytes, tx.codeRate, tx.modulation, tx.powerRaw], [
    77,
    1,
    40,
    5,
...
Deno.test('phy decoders: CSF reports (0xB14E aperiodic, 0xB14D periodic by report type)', () => {
  assertEquals(value(decodeB14E(b14e(1, 2, 11, 9, 12))), {
    sfn: 0,
    subframe: 0,
    carrier: 1,
    ri: 2,
...
Deno.test('phy decoders: NR measurement (Q7), PDSCH status with the widened fields, and PDSCH stats', () => {
  ...
  const [c] = value(
    decodeB97F(
      b97f(174_000, 0, 3, [{ pci: 3, rsrp: -101.5, rsrq: -11.25, beams: 2 }, {
        pci: 8,
        rsrp: -110,
        rsrq: -15,
        beams: 1,
  ...
  assertEquals([c.arfcn, c.ccId, c.servingPci, c.cells.map((x) => [x.pci, x.rsrp, x.rsrq])], [174_000, 0, 3, [[
    3,
    -101.5,
    -11.25,
  ], [8, -110, -15]]]);
  ...
  assertEquals([w.mcs, w.nRb, w.tbsBytes, w.layers, w.slot, w.crcOk, w.harq, w.pci], [
    27,
    217,
    140_000,
    4,
  ...
  assertEquals(value(decodeB888(b888({ decodes: 10, pass: 9, fail: 1, passBytes: 5_000_000_000, failBytes: 10 }))), {
    ...
    crcFail: 1,
    retx: 0,
    passBytes: 5_000_000_000,
    failBytes: 10,
    tbBytes: 5_000_000_010,
  });
});
...
Deno.test('phy extractor: strict versions, bins per (second, carrier), carriers, RACH, and plain data', () => {
  const records: LogRecord[] = [
    ...
    rec(
      0xb193,
      100,
      b193([{ earfcn: 1_000, pci: 7, serving: true, carrier: 0, rxMap: 0b1111, rsrp: -90 }, {
        earfcn: 5_100,
        pci: 8,
        serving: true,
        carrier: 1,
  ...
  assert(
    miss.reason.startsWith('Not decodable (version 48): FieldTap decodes only version 50') &&
      miss.reason.includes('2 records skipped'),
    miss.reason,
  );
  ...
  assertEquals(by.lte_dl_bler.samples.map((s) => [s.tMs, s.carrier, s.value]), [[500, 0, 50], [500, 1, 0], [
    1_500,
    0,
    0,
  ]]);
  ...
  assertEquals(by.lte_dl_layers.samples.map((s) => [s.value, s.tag ?? null]), [[1, null], [1, null], [4, 'TxD'], [
    1,
    null,
  ]]);
  ...
  assertEquals(cap.summary.scellActivity, [{
    index: 1,
    earfcn: 5_100,
    pci: 8,
    firstMs: 100,
...
Deno.test('phy extractor: with a TBS table, UL MCS is derived and the table checks run', () => {
  ...
  const records = [
    rec(0xb139, 10, b139(0, 10, 100, 1, 100)),
    rec(0xb139, 11, b139(0, 10, 100, 2, 100)),
    rec(0xb173, 20, b173(0, [{ mcs: 9, nrb: 10, tbs: 100, qm: 2, crc: true }])),
  ];
...
// -------------------------------------------------------------- the decoders added after the v1 goldens

/** 0xB126: 8-byte header and 20 fixed 48-byte sub-records, oldest first. */
function b126(
  subs: { sfn: number; sf: number; tx: number; rx: number; rank: number; prbs: number[] }[],
  version = 163,
): Uint8Array {
  const b = new Uint8Array(968);
  b[0] = version;
  subs.slice(0, 20).forEach((s, k) => {
    const o = 8 + 48 * k;
    const w = ((s.sfn & 0x3ff) << 4) | (s.sf & 15);
    b[o] = w & 0xff;
    b[o + 1] = w >> 8;
    b[o + 2] = ((s.tx & 7) << 1) | ((s.rx - 1) << 4);
    b[o + 4] = (s.rank - 1) & 3;
    for (const prb of s.prbs) b[o + 8 + (prb >> 3)] |= 1 << (prb & 7);
  });
  return b;
}

/** 0xB12A: 16-byte header and 20 8-byte elements; `cfi` null means the PCFICH was not decoded. */
function b12a(sfn: number, cfis: (number | null)[], version = 161): Uint8Array {
  const b = new Uint8Array(176);
  b[0] = version;
  b[4] = sfn & 0xff;
  b[5] = (sfn >> 8) & 3;
  for (let k = 0; k < 20; k++) {
    const o = 16 + 8 * k, cfi = cfis[k % cfis.length];
    b[o] = k;
    b[o + 2] = cfi === null ? 0 : 1;
    b[o + 3] = cfi === null ? 0 : 4 * cfi;
    b[o + 5] = (k % 10) << 0; // subframe in bits 8-11 of the u16 at +4
  }
  return b;
}

/** 0xB16C: 4-byte header, then elements of (u32 flags, 16-byte grants, 8-byte assignments). */
function b16c(
  elements: {
    sfn: number;
    sf: number;
    grants: { startRb: number; nRb: number; modulation: number }[];
    assignments: number;
  }[],
  version = 50,
): Uint8Array {
  const out = new Bytes().u8(version).u8((elements.length & 0x3) << 6).u8(elements.length >> 2).u8(0);
  for (const e of elements) {
    out.u32((e.sfn & 0x3ff) | ((e.sf & 15) << 10) | ((e.grants.length & 3) << 14) | ((e.assignments & 7) << 17));
    for (const g of e.grants) {
      const grant = new Uint8Array(16);
      grant[4] = g.modulation & 7;
      // start RB at record bits 43-49 and the RB count at 50-56 (the u32 at +5 >> 3 and at +6 >> 2).
      const packed = ((g.nRb & 0x7f) << 7) | (g.startRb & 0x7f);
      grant[5] = (packed << 3) & 0xff;
      grant[6] = (packed >> 5) & 0xff;
      grant[7] = (packed >> 13) & 0xff;
      out.bytes(grant);
    }
    for (let i = 0; i < e.assignments; i++) out.zeros(8);
  }
  return out.done();
}

/** 0xB179: 28-byte header and 12-byte neighbours; RSRP/RSRQ are x = (dBm + 180) * 16 and (dB + 30) * 16. */
function b179(
  earfcn: number,
  pci: number,
  tti: number,
  rsrp: number,
  rsrq: number,
  neighbours: { pci: number; rsrp: number; rsrq: number }[],
  version = 56,
): Uint8Array {
  const p = (v: number) => Math.round((v + 180) * 16), q = (v: number) => Math.round((v + 30) * 16);
  const out = new Bytes().u8(version).zeros(3).u32(9).u32(earfcn).u16(pci).u16(tti)
    .u16(p(rsrp)).u16(p(rsrp)).u16(q(rsrq)).u16(q(rsrq)).u32(neighbours.length);
  for (const n of neighbours) out.u16(n.pci).u16(p(n.rsrp)).u16(p(n.rsrp)).u16(q(n.rsrq)).u16(q(n.rsrq)).u16(0);
  return out.done();
}

/** 0x184C: blocks of a 16-byte header plus 120-byte sub-records. */
function x184c(
  blocks: { counter: number; chains: { chain: number; state: number; powerDbm: number; limitDbm: number }[] }[],
  version = 0x11,
): Uint8Array {
  const bytes: number[] = [];
  blocks.forEach((blk, i) => {
    ...
    h[0] = version;
    h[1] = i === 0 ? blocks.length : 0;
    const w = ((Math.floor(blk.counter / 10) & 0x3ff) << 8) | ((blk.counter % 10) << 4);
    h[7] = w & 0xff;
    h[8] = (w >> 8) & 0xff;
    bytes.push(...h);
    for (const c of blk.chains) {
      const sub = new Uint8Array(120);
      sub[0] = c.chain;
      sub[1] = c.state;
      const raw = Math.round(c.powerDbm * 10) & 0xffff;
      sub[4] = raw & 0xff;
      sub[5] = (raw >> 8) & 0xff;
      const limit = Math.round(c.limitDbm * 10);
      for (const o of [66, 68, 70]) {
        sub[o] = limit & 0xff;
        sub[o + 1] = (limit >> 8) & 0xff;
      }
      bytes.push(...sub);
    }
  });
...
/** 0x1D0B: the two clocks at +4 and +8 and the sequence number at +84. */
function x1d0b(ticks1024: number, ticks19M2: number, sequence: number, version = 7): Uint8Array {
  const b = new Bytes().u32(version).u32(ticks1024).u32(ticks19M2).zeros(72).u32(sequence).zeros(370 - 88).done();
  return b;
}

Deno.test('phy decoders: 0xB126 sub-records, the PRB bitmap as a mask, and version 163 only', () => {
  const subs = value(decodeB126(b126([
    { sfn: 1023, sf: 9, tx: 4, rx: 4, rank: 2, prbs: [0, 1, 49] },
    { sfn: 5, sf: 3, tx: 2, rx: 1, rank: 1, prbs: [] },
  ])));
  assertEquals(subs.length, 20, '20 sub-records, always');
  assertEquals([subs[0].sfn, subs[0].subframe, subs[0].txAntennas, subs[0].rxAntennas, subs[0].rank, subs[0].nPrb], [
    1023,
    9,
    4,
    4,
  ...
  assertEquals(subs[0].prbMask, [0b11, 1 << 17], 'PRB 0, 1 and 49: low word first');
  assertEquals([subs[1].txAntennas, subs[1].rxAntennas, subs[1].rank, subs[1].nPrb], [2, 1, 1, 0]);
  assertEquals(decodeB126(b126([], 162)), { kind: 'versionMiss', key: '0xB126 v162', version: 'v162' });
  assertEquals(decodeB126(b126([]).subarray(0, 900)).kind, 'malformed', 'the body is a fixed 968 bytes');
});

Deno.test('phy decoders: 0xB12A reads 4 x CFI and refuses anything that is not 4, 8 or 12', () => {
  const r = value(decodeB12A(b12a(700, [1, 2, 3, null])));
  assertEquals(r.sfn, 700);
  assertEquals(r.subframes.length, 20);
  assertEquals(r.subframes.slice(0, 4).map((s) => [s.cfi, s.decoded, s.consistent]), [
    [1, true, true],
    [2, true, true],
    [3, true, true],
    [null, false, true],
  ]);
  // CFI 4 does not exist on a 50-PRB cell, and a non-zero field with the flag clear is not consistent.
  const odd = b12a(1, [1]);
  odd[16 + 3] = 16;
  assertEquals([value(decodeB12A(odd)).subframes[0].cfi, value(decodeB12A(odd)).subframes[0].consistent], [
    null,
    false,
  ]);
  assertEquals(decodeB12A(b12a(1, [1], 160)).kind, 'versionMiss');
});

Deno.test('phy decoders: 0xB16C uplink grants, the assignment count, and the chain closing exactly', () => {
  const d = value(decodeB16C(b16c([
    { sfn: 100, sf: 3, grants: [{ startRb: 20, nRb: 6, modulation: 1 }], assignments: 2 },
    { sfn: 100, sf: 4, grants: [], assignments: 1 },
  ])));
  assertEquals(d.declared, 2);
  assertEquals(d.exact, true, 'the flag-driven chain consumes the body');
  assertEquals([d.subframes[0].sfn, d.subframes[0].subframe, d.subframes[0].tti], [100, 3, 1003]);
  assertEquals(d.subframes[0].uplinkGrants, [{ startRb: 20, nRb: 6, modulation: 1 }]);
  assertEquals([d.subframes[0].downlinkAssignments, d.subframes[1].downlinkAssignments], [2, 1]);
  assertEquals(decodeB16C(b16c([], 49)).kind, 'versionMiss');
});

Deno.test('phy decoders: 0xB179 neighbours, the length identity, and the in-record TTI', () => {
  const d = value(decodeB179(b179(1_000, 7, 1_234, -95.5, -11, [{ pci: 11, rsrp: -100.25, rsrq: -14.5 }])));
  assertEquals([d.earfcn, d.pci, d.tti, d.sfn, d.subframe], [1_000, 7, 1_234, 123, 4]);
  assertEquals([d.rsrp, d.rsrq], [-95.5, -11]);
  assertEquals(d.neighbours, [{ pci: 11, rsrp: -100.25, rsrq: -14.5 }]);
  // A body the neighbour count does not explain is malformed, never read to the end of what is there.
  const short = b179(1_000, 7, 1, -95.5, -11, [{ pci: 11, rsrp: -100, rsrq: -14 }]).subarray(0, 32);
  assertEquals(decodeB179(short).kind, 'malformed');
  assertEquals(decodeB179(b179(1, 1, 1, -95, -11, [], 55)).kind, 'versionMiss');
});

Deno.test('phy decoders: 0x184C chains with the off sentinel and its own limits; 0x1D0B clocks', () => {
  const r = value(decodeX184C(x184c([
    {
      counter: 941,
      chains: [{ chain: 0x20, state: 0x24, powerDbm: 21.7, limitDbm: 21.7 }, {
        chain: 0x21,
        state: 0x24,
        powerDbm: -70,
        limitDbm: 24.5,
    ...
    { counter: 942, chains: [{ chain: 0x20, state: 0x30, powerDbm: 12, limitDbm: 24.5 }] },
  ])));
  assertEquals(r.exact, true);
  assertEquals(r.blocks, 2);
  assertEquals(r.blockCounters, [941, 942], "consecutive subframes of the front end's own counter");
  assertEquals(r.samples.map((s) => [s.chain, s.powerDbm, s.live]), [[0x20, 21.7, true], [0x21, -70, false], [
    0x20,
    12,
    true,
  ]]);
  assertEquals(r.samples[0].limitsDbm, [21.7, 21.7, 21.7]);
  assertEquals(bindingLimitDbm(r.samples[1]), 24.5);
  assertEquals(decodeX184C(x184c([{ counter: 1, chains: [{ chain: 0, state: 0, powerDbm: 0, limitDbm: 0 }] }], 0x10)), {
    kind: 'versionMiss',
    key: '0x184C v16',
    version: 'v16',
  });
  const clock = value(decodeX1D0B(x1d0b(10_000, 500_000, 42)));
  assertEquals(clock, { ticks1024: 10_000, ticks19M2: 500_000, sequence: 42 });
  assertEquals(decodeX1D0B(x1d0b(1, 1, 1, 6)), { kind: 'versionMiss', key: '0x1D0B v6', version: 'v6' });
  assertEquals(decodeX1D0B(x1d0b(1, 1, 1).subarray(0, 40)).kind, 'malformed');
});
